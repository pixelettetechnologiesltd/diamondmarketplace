...
loadCultureFiles();
function createSlider(min, max, start, end, step, tooltipChangeHandler, renderingTicksHandler, values) {
...
let colorSlider = createSlider(0, colors.length - 1, 'D', 'I', 1, tooltipChangeHandler, renderingTicksHandler, colors);
let claritySlider = createSlider(0, clarities.length - 1, 'IF', 'VS2', 1, tooltipChangeHandler, renderingTicksHandler, clarities);
...
function createSliderCarat(start, end, step) {
  const values = Array.from(Array((end - start) * 2 + 1).keys()).map(x => start + x * step);
