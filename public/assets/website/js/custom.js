...
$(document).ready(function () {
  ...
  var final_counter_time =
    difference_in_sec - difference_in_current_start_seceond;
  ...
  function getAllNfts(cat_id, col_id, l) {
    ...
    var grtCollNftsUrl =
      base_uri + "/ajax_all_nfts/" + cat_id + "/" + col_id + "/" + l;
  ...
  $(document).on(
    "change",
    ".uploadProfileInput1, .uploadProfileInput",
    function () {
  ...
  $(document).on("keyup", "#collectionName", function () {
    ...
    if (collection.length > 5) {
      $.ajax({
        ...
        success: function (res) {
          if (res.status == "success") {
            ...
            $(".collection-name-check").html(
              '<i class="fa fa-check"></i> ' + res.msg
            );
          } else {
            $(".collection-name-check").addClass(res.class);
            $(".collection-name-check").html(
              '<i class="fa fa-xmark"></i> ' + res.msg
            );
          }
        },
      });
      ...
      $(".collection-name-check").html(
        '<i class="fa fa-xmark"></i> Please enter minimum 6 characters'
      );
    }
  });
  ...
  function appendUserRow(id, user) {
    var html =
      '<div id="opt-row.' +
      id +
      '" class="form-group row align-items-end mb-3 g-1">\n' +
      '            <div class="col">\n' +
      '                <input required type="text" class="form-control" id="opt-type.' +
      id +
      '" name="opt_type[]" placeholder="Ex: White" value="' +
      user.type +
      '">\n' +
      "            </div>\n" +
      '            <div class="col">\n' +
      '            <div class="input-group">\n' +
      '                <input required type="text" class="form-control"  id="opt-name.' +
      id +
      '" name="opt_val[]" placeholder="Ex: 20%" value="' +
      user.name +
      '">\n' +
      ...
      "             <button delet-id=" +
      id +
      ' type="button"  class="btn btn-danger deleteRow">Delete</button>\n' +
      "            </div>\n" +
      "        </div>";
