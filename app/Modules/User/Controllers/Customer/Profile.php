...
class Profile extends BaseController {
    ...
    */ 
	...
	{  
        ...
        if ($this->validation->withRequest($this->request)->run()) {
            $portaddress        = $this->request->getFile('port_address',FILTER_SANITIZE_STRING);
            $savepath="public/uploads/dashboard/new/";
            $old_port_address       = $this->request->getVar('old_port_address', FILTER_SANITIZE_STRING);
            if($this->request->getMethod() == "post"){
                $portaddress       =   $this->imagelibrary->image($portaddress,$savepath,$old_port_address,300,300);
            }
            $userdata = array( 
                'full_name'      => $this->request->getVar('full_name',FILTER_SANITIZE_STRING),
                'Dob'      => $this->request->getVar('Dob',FILTER_SANITIZE_STRING), 
                'Residential_Address'      => $this->request->getVar('Residential_Address',FILTER_SANITIZE_STRING), 
                'ID_number'      => $this->request->getVar('ID_number',FILTER_SANITIZE_STRING), 
                'port_address'    => $this->request->getVar('old_port_address',FILTER_SANITIZE_STRING),
    ...
    */
    ...
    public function withdraw()
    {
        ...
        $this->validation->setRule('Business_Name', 'Business Name', 'required');
        $this->validation->setRule('Registration_No', 'Registration No', 'required');
        $this->validation->setRule('Business_Address', 'Business  Address', 'required');
        $this->validation->setRule('detail', 'detail', 'required');
        $this->validation->setRule('Business_Activities', 'Business Activities', 'required');
        $this->validation->setRule('Source_of_Income', 'Source of Income', 'required');
        
        if ($this->validation->withRequest($this->request)->run()) {
            $portaddress        = $this->request->getFile('port_addresskyb',FILTER_SANITIZE_STRING);
            $savepath="public/uploads/dashboard/new/";
            $old_port_address       = $this->request->getVar('old_port_addresskyb', FILTER_SANITIZE_STRING);
            if($this->request->getMethod() == "post"){
                $portaddress       =   $this->imagelibrary->image($portaddress,$savepath,$old_port_address,300,300);
            }
            
            $userdata = array( 
                'Business_Name'      => $this->request->getVar('Business_Name',FILTER_SANITIZE_STRING),
                'Registration_No'      => $this->request->getVar('Registration_No',FILTER_SANITIZE_STRING), 
                'Business_Address'      => $this->request->getVar('Business_Address',FILTER_SANITIZE_STRING), 
                'detail'      => $this->request->getVar('detail',FILTER_SANITIZE_STRING), 
                'Business_Activities'      => $this->request->getVar('Business_Activities',FILTER_SANITIZE_STRING),
                'Source_of_Income'      => $this->request->getVar('Source_of_Income',FILTER_SANITIZE_STRING),
                'port_addresskyb'    => $this->request->getVar('old_port_addresskyb',FILTER_SANITIZE_STRING),
            );
            $update = $this->common_model->update('user', $where, $userdata);
            if($update){
                $this->session->setFlashdata('message',display('update_successfully'));
