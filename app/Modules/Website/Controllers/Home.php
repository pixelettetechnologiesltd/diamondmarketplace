...
class Home extends BaseController
{
    ...
    public function ajax_coll_nfts_filter($collectionId = null, $color = null, $clarity = null, $carat = null, $cut = null, $price = null, $loadmore = 1)
    {
        $limit = 20;
        $data['nfts'] = $this->web_model->getNfts(['nfts_store.collection_id' => $collectionId, 'nfts_store.color >= ' => $color[0], 'nfts_store.color <=' => $color[2], 'nfts_store.clarity >= ' => $clarity[0], 'nfts_store.clarity <=' => $clarity[2], 'nfts_store.carat >= ' => $carat[0], 'nfts_store.carat <=' => $carat[2], 'nfts_store.cut >= ' => $cut[0], 'nfts_store.cut <=' => $cut[2], 'nfts_store.price >= ' => $price[0], 'nfts_store.price <=' => $price[2]], ($loadmore * $limit));
        $result = view('themes/' . $this->templte_name->name . '/ajax_nfts', $data);
        echo json_encode(['status' => true, 'data' => $result]);
    }
